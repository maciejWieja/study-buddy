...
export const handlers = [
  ...
  rest.post('/students/search', (req, res, ctx) => {
    const students = req.body.searchPhrase ? db.student.getAll() : [];
    const matchingStudents = students.filter(({ name }) => name.toLowerCase().includes(req.body.searchPhrase.toLowerCase()));
    return res(
      ctx.status(200),
      ctx.json({
        students: matchingStudents,
